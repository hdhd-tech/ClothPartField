...
def extract_largest_component(
    ...
    # Debug Panel 2 specifically
    if color_id == 2:
        print(f"=== Panel 2 Debug Info ===")
        print(f"  Original mesh: {len(cc_mesh.faces)} faces, {len(cc_mesh.vertices)} vertices")
        print(f"  Largest component: {len(largest_cc_mesh.faces)} faces, {len(largest_cc_mesh.vertices)} vertices")
        print(f"  Is watertight: {largest_cc_mesh.is_watertight}")
        print(f"  Before boundary extraction...")
...
def compute_edge_labels(
    ...
    for panel in panels:
        ...
        # Debug Panel 2 specifically
        if panel.color_id == 2:
            print(f"\n=== Debugging Panel {panel.color_id} ===")
            print(f"Number of boundary edges: {len(panel.boundary_edges)}")
            print(f"Number of boundary vertices: {len(panel.boundary_vertices)}")
            print(f"Mesh vertices shape: {panel.mesh.vertices.shape}")
            print(f"Boundary edges: {panel.boundary_edges}")
...
def main():
    ...
    # Process mesh
    panels, edge_labels, kdtree = process_mesh("test_data/dress_colored_vert_newest.ply")
