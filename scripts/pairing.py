...
from typedef import Segment, ColorPanel
...
def get_segment_edges(
    panel_id: int, segment: Segment, panels: List[ColorPanel], kdtree: KDTree
) -> Set[Tuple[int, int]]:
    """Get all global edges for a segment."""
    be = panels[panel_id].boundary_edges
    cc = panels[panel_id].mesh
    segment_edges = set()
...
def find_best_match(
    ...
    segment_edges = get_segment_edges(panel_id, segment, panels, kdtree)
    best_match = None
    best_overlap = 0
    ...
    for other_panel_id, other_segments in enumerate(all_segments):
        ...
        for other_seg_idx, other_segment in enumerate(other_segments):
            ...
            other_segment_edges = get_segment_edges(
                other_panel_id, other_segment, panels, kdtree
            )
    ...
    for panel_id, segments in enumerate(all_segments):
        for seg_idx, segment in enumerate(segments):
            ...
            best_match = find_best_match(
                panel_id, segment, all_segments, panels, kdtree, used_segments
            )
...
def plot_panel_with_segment(
    ...
    cc = panel_data.mesh
    bv = panel_data.boundary_vertices
...
def plot_sewing_pairs(
    sewing_pairs: List, panels: List[ColorPanel], save_dir: str = "../debug/test_data"
):
    ...
    for pair_idx, (seg1_data, seg2_data) in enumerate(sewing_pairs):
        ...
        panel1_id, _, _, reverse1 = seg1_data
        panel2_id, _, _, reverse2 = seg2_data

        plot_panel_with_segment(ax1, panels[panel1_id], seg1_data, pair_idx, reverse1)
        plot_panel_with_segment(ax2, panels[panel2_id], seg2_data, pair_idx, reverse2)
