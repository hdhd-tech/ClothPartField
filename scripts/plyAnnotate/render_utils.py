...
class SimpleShader(ShaderBase):
    def forward(self, fragments, meshes, **kwargs) -> torch.Tensor:
        ...
        import dataclasses
        new_fragments = dataclasses.replace(fragments, bary_coords=quantized_bary_coords)
        # fragments.bary_coords = quantized_bary_coords

        # fragments.bary_coords[:,:,:,:,0]=1
        # fragments.bary_coords[:,:,:,:,1]=0
        # fragments.bary_coords[:,:,:,:,2]=0
        
        texels = meshes.sample_textures(new_fragments)
...
class CameraSeed:
    def __init__(
        ...
        Args:
            ...
            verts_rgb (torch.Tensor, optional): 形状为 (B, N_v, 3) 或 (1, N_v, 3) 的顶点颜色张量。
                                                 如果提供，将作为 TexturesVertex 使用。
        """

        self.device = DEVICE

        self.translation = (
            torch.tensor(translation, dtype=torch.float32, device=self.device)
        ...
        self.principal_point = (
            torch.tensor(principal_point, dtype=torch.float32, device=self.device)
            if principal_point is not None
            else None
        )
        self.img_hw = img_hw  # img_hw 不需要是 tensor，保持list或tuple
        self.verts_rgb = verts_rgb  # 保存顶点颜色
...
class PhongRenderer(nn.Module):
    def __init__(self, ):
        ...
        self.mesh_renderer = MeshRenderer(
            rasterizer=self.rasterizer, shader=self.phong_shader
        )
