...
def _to_rel(p0, p1, p):
    ...
    rel0 = np.dot(p - p0, vdir) / np.dot(vdir, vdir)
    p_proj = p0 + vdir * rel0
    p_diff = p - p_proj
    rel1 = (p_diff / vorth)[0]
    return [rel0, rel1]
...
def make_spec(panels: List[ColorPanel], sewing_pairs: List[Tuple]):
    ...
    for i, panel in enumerate(panels):
        ...
        for j, (_, e0, e1) in enumerate(p2s[i]):
            if e1 < e0:
                e1 += len(panel.boundary_vertices)
            if e1 >= len(panel.boundary_vertices) - 1:
                vids = (
            ...
            panel_json[f"panel_{i}"]["vertices"].append(segment_points[0])

        if panel_json[f"panel_{i}"]["edges"]:
            panel_json[f"panel_{i}"]["edges"][-1]["endpoints"][1] = 0
    ...
    for _, (s0, s1) in s2p.items():
        stitches_json.append(
            [
                {"panel": f"panel_{s0[0]}", "edge": s0[1]},
                {"panel": f"panel_{s1[0]}", "edge": s1[1]},
            ]
        )
